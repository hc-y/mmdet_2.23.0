...
def cluster_gt_bboxes(p_bboxes):
    ...
    """
    num_p = len(p_bboxes)
    # Step 1. compute giou (distance) between all gt bboxes, and adjust the value interval from [-1,1] to [0,2] via (1. - giou);
    dist_gious = 1. - bbox_overlaps(p_bboxes, p_bboxes, mode='giou', is_aligned=False, eps=1e-7)
    # Step 2. select the \k smallest dist_gious as candidates, and compute the mean and std, set mean + std * 0.8 as the dist_gious threshold (cuttoff_distance);
    selectable_k = int(num_p * 2)  # 超参数, 待调节;
    triu_inds = torch.triu_indices(num_p, num_p, 1)
    candidate_dist_gious = dist_gious[triu_inds[0], triu_inds[1]].sort(descending=False)[0][:selectable_k]
    candidate_dist_gious_thr = candidate_dist_gious.mean() + candidate_dist_gious.std() * 1.0  # 超参数, 待调节;
    # Step 3. compute local density \rho for each gt bbox; 使用了 gaussian kernel 连续型密度;
    p_rho = (torch.exp(-(dist_gious / candidate_dist_gious_thr).square()) * (dist_gious <= candidate_dist_gious_thr)).sum(dim=1) - 1.  # 减去1是因为不包括与其自身之间的giou
    # 删除dist_gious_triu = torch.triu(dist_gious, diagonal=1)
    # 删除rho_triu = torch.exp(-(dist_gious.triu(diagonal=1) / candidate_dist_gious_thr).square()) * torch.ones_like(dist_gious).triu(diagonal=1)
    # 删除rho = rho_triu.sum(dim=1)
    # Step 4. compute high local density distance \sigma for each gt bbox;
    hld_mask = p_rho.expand_as(dist_gious) > p_rho[:, None].expand_as(dist_gious)
    p_sigma = (hld_mask * dist_gious + ~hld_mask * 2.).min(dim=1)[0]
    p_sigma[p_rho.argmax()] = dist_gious.max()  # 对于最大局部密度点, 设置 \sigma 为 dist_gious 的最大值dist_gious.max()
    # Step 5. determine the cluster center according to \rho and \sigma;
    cluster_1st = (p_rho * p_sigma).sort(descending=True)[1][:2]  # 确定了2个簇中心
    cluster_label = torch.zeros((num_p, 2), dtype=torch.int64, device=dist_gious.device)
    cluster_label[cluster_1st, 0] = torch.arange(len(cluster_1st), device=dist_gious.device) + 1
    ...
    if flag_cluster == 'flag_cluster_v1':
        ...
        for _sorted_ind in p_rho_sorted_ind:
            if cluster_label[_sorted_ind, 0] == 0:
                _min_dist_gious, _argmin_dist_gious = dist_gious[_sorted_ind, p_rho_clustered_ind].min(dim=0)
                if _min_dist_gious <= candidate_dist_gious_thr * 1.0:
                    cluster_label[_sorted_ind, 0] = cluster_label[p_rho_clustered_ind[_argmin_dist_gious], 0]
            else:
        ...
        if _ind_rest_1.numel() > 0:
            _min_dist_gious, _argmin_dist_gious = dist_gious[_ind_rest_1][:, cluster_1st].min(dim=1)
            _cluster_2nd_mask = _min_dist_gious < candidate_dist_gious_thr * 1.0  # 超参数, 待调节;
            cluster_label[_ind_rest_1[_cluster_2nd_mask], 0] = cluster_label[cluster_1st, 0][_argmin_dist_gious[_cluster_2nd_mask]]
            ...
            if cluster_2nd.numel() > 0 and _ind_rest_2.numel() > 0:
                _min_dist_gious, _argmin_dist_gious = dist_gious[_ind_rest_2][:, cluster_2nd].min(dim=1)
                _cluster_3rd_mask = _min_dist_gious < candidate_dist_gious_thr * 1.0  # 超参数, 待调节;
                cluster_label[_ind_rest_2[_cluster_3rd_mask], 0] = cluster_label[cluster_2nd, 0][_argmin_dist_gious[_cluster_3rd_mask]]
                ...
                if cluster_3rd.numel() > 0 and _ind_rest_3.numel() > 0:
                    _min_dist_gious, _argmin_dist_gious = dist_gious[_ind_rest_3][:, cluster_3rd].min(dim=1)
                    _cluster_4th_mask = _min_dist_gious < candidate_dist_gious_thr * 1.0  # 超参数, 待调节;
                    cluster_label[_ind_rest_3[_cluster_4th_mask], 0] = cluster_label[cluster_3rd, 0][_argmin_dist_gious[_cluster_4th_mask]]
...
def main():
    ...
    _gt_bboxes = l[_ind_cls2_l, 1:]  # normalized xywh
    ...
    _cluster_color = [(0,255,0), (0,0,255)]
    for _cluster_id in [1, 2]:
        ...
        for _ind in _ind_cluster:
            ...
            img_cls_2_draw.chord([_bbox_ctr_x-5, _bbox_ctr_y-5, _bbox_ctr_x+5, _bbox_ctr_y+5], 0, 360, fill=_cluster_color[_cluster_id-1])
            if cluster_label[_ind, 1] == 1:
                img_cls_2_draw.chord([_bbox_ctr_x-2, _bbox_ctr_y-2, _bbox_ctr_x+2, _bbox_ctr_y+2], 0, 360, fill=(255,0,0))

    img_cls2.save(path_to_tmp / f'img_cls2_0.jpg')
    pass
