# hc-y_modify1103:
# CUDA_VISIBLE_DEVICES=4,5,6,7 bash ./tools/dist_train.sh configs/yolof/yolof_r50_c5_8x8_1x_argoverse_hd.py 4 
_base_ = [
...
model = dict(
    ...
    backbone=dict(
        ...
        out_indices=(3, ),
        frozen_stages=1,
        norm_cfg=dict(type='BN', requires_grad=False),  # hc-y_Q1220:为什么有的模型设置ResNet中BN.requires_grad=False, 有的模型设置ResNet中BN.requires_grad=True?
        norm_eval=True,
        style='caffe',
        init_cfg=dict(
            type='Pretrained',
...
optimizer = dict(
    type='SGD',
    lr=0.12*8*4/(8*8),  # hc-y_modify1103:原始为lr=0.12 with num_GPU=8, samles_per_gpu=8; num_GPU=4, samples_per_gpu=6时, 则lr乘以4*6/(8*8);
    momentum=0.9,
    weight_decay=0.0001,
    paramwise_cfg=dict(
...
train_pipeline = [
    ...
    dict(type='Resize', img_scale=(1024, 640), keep_ratio=True),  # (1280, 800), (1024, 640)
...
test_pipeline = [
    ...
    dict(
        ...
        img_scale=(1024, 640),  # (1280, 800), (1024, 640)
        flip=False,
        transforms=[
...
data = dict(
    samples_per_gpu=8,
    workers_per_gpu=8,  # hc-y_modify1103:12 12, 8 8, 2 4; on local 1050, 1, 2;
