# hc-y_modify1103:
# CUDA_VISIBLE_DEVICES=4,5,6,7 bash ./tools/dist_train2.sh configs/yolof/yolofmbranchv2v2_r50_c5_8x8_2x_argoverse_hd_poc_grad.py 4 
_base_ = [
    '../_base_/datasets/argoverse_hd_detection.py',
    '../_base_/schedules/schedule_2x.py', '../_base_/default_runtime.py'
]
model = dict(
    type='YOLOFMBranchV1v2',
    backbone=dict(
        type='ResNet',
        depth=50,
        num_stages=4,
...
train_pipeline = [
    ...
    dict(type='ResizeChipsV1v2', img_scale=(1280, 800), keep_ratio=True),  # (1280, 800), (1024, 640)
    dict(type='RandomFlip', flip_ratio=0.),
    # dict(type='RandomShift', shift_ratio=0.5, max_shift_px=32),
    dict(type='NormalizeChipsV1v1', **img_norm_cfg),
    dict(type='Pad', size_divisor=32),
    dict(type='DefaultFormatBundleChipsV1v2'),
    dict(type='Collect', keys=['img', 'gt_bboxes', 'gt_labels', 'chip4', 'chip3_fts_idx'])
]
test_pipeline = [
    dict(type='LoadImageFromFile'),
    dict(
        ...
        flip=False,
        transforms=[
            dict(type='ResizeChipsV1v2', keep_ratio=True),
            dict(type='RandomFlip'),
            dict(type='NormalizeChipsV1v1', **img_norm_cfg),
            dict(type='Pad', size_divisor=32),
            dict(type='ImageToTensorChipsV1v2', keys=['img']),
            dict(type='Collect', keys=['img', 'chip4', 'chip3_fts_idx']),
        ])
]
data = dict(
    samples_per_gpu=2,
...
runner = dict(type='EpochBasedRunner', max_epochs=24)  # configs/_base_/schedules/schedule_1x.py
