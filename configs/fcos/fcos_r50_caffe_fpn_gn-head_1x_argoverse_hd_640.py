...
data = dict(
    samples_per_gpu=6,  # hc-y_modify1031:设置为6时, GPU-Util=80%~87%; 设置为8时, CUDA out of memory;
    workers_per_gpu=6,  # hc-y_modify1103:10 10, 8 8, 2 4
    train=dict(pipeline=train_pipeline),
    val=dict(pipeline=test_pipeline),
    test=dict(pipeline=test_pipeline))
# optimizer
optimizer = dict(  # hc-y_modify1031:原始为lr=0.01 with num_GPU=8, samles_per_gpu=2; num_GPU=4, samples_per_gpu=6时, 则lr乘以4*6/(8*2);
    lr=0.01*6*4/(2*8), paramwise_cfg=dict(bias_lr_mult=2., bias_decay_mult=0.))
