...
train_pipeline = [
    dict(type='Mosaic', img_scale=img_scale, pad_val=114.0),  # hc-y_note0420:做Ablation Study时禁用 Mosaic, RandomAffine, MixUp, 以减少随机性;
    dict(
        type='RandomAffine',
...
train_dataset = dict(
    ...
    dataset=dict(
        type=dataset_type,
        ann_file=data_root + 'annotations/train.json',  # annotations_focus/train_5lft_euc_1.2_3.2.json; annotations/train.json;
...
data = dict(
    ...
    test=dict(
        type=dataset_type,
        ann_file=data_root + 'annotations/val.json',  # annotations_focus/val_5lft_euc_1.2_3.2.json; annotations/val.json;
        img_prefix=data_root + 'images/',
        pipeline=test_pipeline))
